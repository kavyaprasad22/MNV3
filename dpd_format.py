...
class Bneck(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        return x
    def get_config(self):
        config = super(Bneck, self).get_config()
...
class DPD(tf.keras.layers.Layer):
    def __init__(self,
                 ...
                 activation=tf.nn.relu6,
                 **kwargs):
        super(DPD, self).__init__(**kwargs)
        self.filters = _make_divisible(filters * alpha, 8)
        self.shape=shape
        self.expansion_filters = _make_divisible(expansion_filters * alpha, self.shape)
        self.kernel_size = kernel_size
        self.strides = strides
        self.activation = activation
        self.expand_conv2d = tf.keras.layers.DepthwiseConv2D(self.kernel_size, strides=self.strides, use_bias=False,padding='same' if self.strides == 1 else 'valid',depth_multiplier=self.expansion_filters//self.shape,depthwise_regularizer=tf.keras.regularizers.l2(1e-5))
        self.expand_bn = tf.keras.layers.BatchNormalization()
        self.zero_padding2d = tf.keras.layers.ZeroPadding2D(((self.kernel_size - 1) // 2, (self.kernel_size - 1) // 2))
        self.depthwise_conv2d = tf.keras.layers.DepthwiseConv2D(self.kernel_size, strides=self.strides, use_bias=False,padding='same' if self.strides == 1 else 'valid',depthwise_regularizer=tf.keras.regularizers.l2(1e-5))
        self.depthwise_bn = tf.keras.layers.BatchNormalization()
        self.project_conv2d = tf.keras.layers.Conv2D(self.filters, kernel_size=1, padding='same', use_bias=False,kernel_regularizer=tf.keras.regularizers.l2(1e-5))
        self.project_bn = tf.keras.layers.BatchNormalization()
    ...
    def call(self, inputs):
        ...
        return x
    def get_config(self):
        config = super(DPD, self).get_config()
...
class PDP(tf.keras.layers.Layer):
    def __init__(self,
                 filters,
                 expansion_filters,
                 kernel_size,
                 ...
                 activation=tf.nn.relu6,
                 **kwargs):
        super(PDP, self).__init__(**kwargs)
        self.filters = _make_divisible(filters * alpha, 8)
        self.expansion_filters = expansion_filters
        self.kernel_size = kernel_size
        self.strides = strides
        ...
        self.depthwise_conv2d = tf.keras.layers.DepthwiseConv2D(self.kernel_size, strides=self.strides, use_bias=False,
                                                                padding='same' if self.strides == 1 else 'valid',depthwise_regularizer=tf.keras.regularizers.l2(1e-5))
        self.depthwise_bn = tf.keras.layers.BatchNormalization()
        self.project_conv2d = tf.keras.layers.Conv2D(self.filters, kernel_size=self.kernel_size, padding='same', use_bias=False,kernel_regularizer=tf.keras.regularizers.l2(1e-5))
        self.project_bn = tf.keras.layers.BatchNormalization()
        self.add = tf.keras.layers.Add()
...
def MobilenetV3(input_shape,num_classes, size="large", include_top=True,alpha=1.0):
    ...
    if size == "large":
        ...
        x = tf.keras.layers.Conv2D(first_block_filters, 3, strides=1, padding='same', use_bias=False,kernel_regularizer=tf.keras.regularizers.l2(1e-5))(input)
        x = tf.keras.layers.BatchNormalization()(x)
        x = HSwish()(x)
        x = Bneck(32, 16, 3, alpha=alpha, strides=1, use_se=True, activation=tf.nn.relu6)(x)
        x = DPD(x.shape[-1],40, 88, 3, alpha=alpha, strides=1,  activation=tf.nn.relu6)(x)
        x = DPD(x.shape[-1],40, 240, 3, alpha=alpha, strides=1,  activation=tf.nn.relu6)(x)
        x = Bneck(48, 160, 5, alpha=alpha, strides=1, use_se=True, activation=h_swish)(x)
        x = DPD(x.shape[-1],96, 288, 5, alpha=alpha, strides=2,activation=tf.nn.relu6)(x)
        x = DPD(x.shape[-1],128, 592, 5, alpha=alpha, strides=1, activation=tf.nn.relu6)(x)
        x = tf.keras.layers.Conv2D(_make_divisible(256 * alpha, 8), 1, use_bias=False,kernel_regularizer=tf.keras.regularizers.l2(1e-5))(x)
        x = tf.keras.layers.BatchNormalization()(x)
        x=SeBlock()(x)
        output = HSwish()(x)
    if include_top:
